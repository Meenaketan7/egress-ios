...
/// Standing-water flood zones (`venue.water`) block unconditionally: a static no-go the crowd routes
/// around, exactly like a wall.
public enum BlockedCells {
    public static func of(_ venue: VenueModel) -> Set<GridCoord> {
        ...
        for obstacle in venue.obstacles where obstacle.blocksMovement {
            rasterise(origin: obstacle.origin, size: obstacle.size, of: venue.geometry, into: &blocked)
        }
        for zone in venue.water {
            rasterise(origin: zone.origin, size: zone.size, of: venue.geometry, into: &blocked)
        }
        // The room's exterior (free-form editor): cells outside the walls' enclosed interior, so only
        // the drawn shape is floor. Clamped to the grid; empty for any venue that doesn't set it.
        for cell in venue.blockedCells where venue.geometry.size.contains(cell) {
            blocked.insert(cell)
        }
        return blocked
    }

    /// Every in-bounds cell an axis-aligned box overlaps. The far edges are half-open
    /// (matching `Obstacle.contains` / `WaterZone.contains`), so a box flush with a cell boundary
    /// does not spill into the next cell. Shared by obstacle footprints and water flood zones.
    private static func rasterise(origin: Vec2, size: Vec2, of geometry: GridGeometry, into blocked: inout Set<GridCoord>) {
        let minCell = geometry.cell(for: origin)
        let maxCell = geometry.cell(for: origin + size - Vec2(1e-9, 1e-9))
